...
class WorkSpace(QWidget):
    ...
    def init_ui(self):
        ...
        self.title_label = EditableLabel(
            self,
            self.workspace_json_file.read_data()["card-title"],
            "background:#1e1e1e;",
        )
        ...
        if back_directory is not None:
            ...
            self.go_back_button.clicked.connect(
                lambda: self.go_back_from_workspace.emit(
                    prev_card_data, self.workspace_file[:-6]
                )
            )
        else:
        ...
        for card_data in self.workspace_json_file["cards"]:
            ...
            card.card_selected.connect(
                lambda card_directory, workspace_directroy: self.card_selected.emit(
                    card_directory, workspace_directroy
                )
            )
    ...
    def create_card_directory(self, file_type):
        ...
        card_data = {
            "card-fname": card_fname + "/" + card_fname + ".vnote",
            "type": file_type,
        }
            ...
            j.save_data(
                {
                    "type": "workspace",
                    "cards": [],
                    "back": "/../../../" + os.path.basename(self.workspace_file),
    ...
    def add_widget(self, widget):
        self.gridLayout.addWidget(
            widget, self.next_item_index // 5, self.next_item_index % 5
        )
